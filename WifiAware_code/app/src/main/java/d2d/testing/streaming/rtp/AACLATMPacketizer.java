...
/**
 * Se encarga de leer de un ByteBufferInputStream y solicitar el buffer de un datagrama UDP a RTPSocket para formar un nuevo paquete.
 */
@SuppressLint("NewApi")
		...
		try {
			while (!Thread.interrupted()) {
				...
				if (length>0) {
					...
					/** Send es basicamente socket.commitBuffer(length). */
					send(rtphl+length+4);
